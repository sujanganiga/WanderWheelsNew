...
function UserBookings() {
  ...
  const generateInvoice = (booking) => {
    ...
    doc.text("", 10, 30);
    doc.text(`Car: ${booking.car ? booking.car.name : 'N/A'}`, 10, 50);
    doc.text(`Total hours: ${booking.totalHours}`, 10, 60);
    doc.text(`Rent per hour: ${booking.car ? booking.car.rentPerHour : 'N/A'}`, 10, 70);
    ...
    doc.text(`Date of booking: ${moment(booking.createdAt).format("MMM DD yyyy")}`, 10, 120);
                ...
                >
                  <Col className="bcardata" lg={6} sm={24}>
                    <p className="bcarname">
                      <b>{booking.car ? booking.car.name : 'Car data not available'}</b>
                    </p>
                    <p>
